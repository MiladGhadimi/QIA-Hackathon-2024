...
class BQCProgramParams:
    ...
    def generate_random_params(cls):
        ...
        params.theta2 = 5 * numpy.pi

        params.theta3 = 2 * numpy.pi

        params.theta4 = numpy.random.random() * numpy.pi
        ...
        params.r3 = numpy.random.randint(0, 2)

        params.r4 = numpy.random.randint(0, 2)
...
class ClientProgram(Program):
    ...
    def run(self, context: ProgramContext):
        ...
        p3 = remote_state_preparation(epr_socket, self.theta3)

        p4 = remote_state_preparation(epr_socket, self.theta4)
        ...
        delta2 = (self.theta2 + (p2 + self.r2) * math.pi)
        ...
        # Send third angle to server.

        delta3 = (

           self.theta3 + (p3 + self.r3) * math.pi

        )

        self.logger.info(f"Sending delta3 = {delta3}")

        send_float(csocket, delta3)

 

        # Send fourth angle to server.
        ...
        self.logger.info(f"Sending delta4 = {delta4}")

        send_float(csocket, delta4)
        ...
        m3 = yield from recv_int(csocket)

        self.logger.info(f"Received m3 = {m3}")

 

        # receive results of server measurement

        m4 = yield from recv_int(csocket)

        self.logger.info(f"Received m4 = {m4}")
        ...
        return {
            ...
            "delta3": delta3 / math.pi,

            "delta4": delta4 / math.pi,
...
class ServerProgram(Program):
    ...
    def run(self, context: ProgramContext):
        ...
        q3 = recv_remote_state_preparation(epr_socket)

        q4 = recv_remote_state_preparation(epr_socket)
        ...
        # Apply Oracle at the first


        # q1.rot_Z(int(numpy.random.randint(0,2) * numpy.pi))



        # q4.rot_Z(int(numpy.random.randint(0,2) * numpy.pi))
        ...
        # Creating the circuit
        q2.rot_Z(angle=numpy.pi)

        q3.rot_Z(angle=numpy.pi)

        #q2.rot_Z(-int(numpy.random.randint(0,2) * 0.5*numpy.pi))
        q2.rot_Z(angle=-numpy.pi/2)
        q2.H()
        ...
        # Receive from the client the angle to measure the second qubit in.

        angle = yield from recv_float(csocket)

        self.logger.info(f"Received delta3 = {angle}")

 

        #q3.rot_Z(angle = angle)
        ...
        self.logger.info(f"Sending m3 = {s}")
        ...
        final_dm = get_qubit_state(q4, "Server")
        ...
        self.logger.info(f"Sending m4 = {s}")
...
if __name__ == "__main__":
    ...
    parameter_prog = BQCProgramParams.generate_random_params()
    ...
    theta2 = parameter_prog.theta2

    theta3 = parameter_prog.theta3
    ...
    for i in range(0, int(iteration)):
        ...
        print(list(client_results[0].values())[-3:-1])
        ...
        all_result = list(client_results[0].values())[-3:-1]

        counts.append(all_result)
    ...
    for lst in counts:
        ...
        if tuple_lst in list_counts:

            list_counts[tuple_lst] += 1
    ...
    plt.bar(range(len(list_counts)), list(list_counts.values()), tick_label = [str(lst) for lst in list(list_counts.keys())])
    ...
    plt.title(f'Measurement Results (m2, m3) {theta2, theta3}')
    plt.show()